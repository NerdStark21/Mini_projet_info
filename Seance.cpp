	...
	for (itPage = lesPage.begin(); itPage != lesPage.end(); itPage++)
	{
		...
		for (itImage = lesImages.begin(); itImage != lesImages.end(); itImage++)
		{
			if (!(*itImage).getPresence())
			{
				...
				unEleve.setPrenom(unEleve.getPrenom() + "        Absent");
				listAbsent.push_back(unEleve);
			}
			else
			{
				unEleve = (*itImage).getEleve();
				unEleve.setPrenom(unEleve.getPrenom() + "        Présent");
				listAbsent.push_back(unEleve);
			}
		}
	}
...
void Seance::sauvegarde()
{
	...
	flux << "La variation du taux d'absentéisme par rapport à la séance précédente est de :" << laMemoire.getVariationAbsenteisme() << "%" << endl;
	...
	flux << "Voici la liste des élèves :" << endl << endl;
